    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500">Reachability & latency (auto refresh every {POLL_INTERVAL/1000}s)</p>
          <p className="mt-1 text-xs text-gray-400 flex items-center gap-1"><FiClock className="h-3 w-3"/> Updated {lastUpdatedText}</p>
          {error && <p className="mt-1 text-xs text-rose-600">Error: {error}</p>}
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <button onClick={() => fetchDevices(true)} disabled={isRefreshing} className="flex items-center gap-2 px-4 py-2 bg-white border rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50">
      ...
      <div className="bg-white border rounded-lg shadow-sm overflow-hidden">
        <div className="p-4 border-b flex items-center justify-between bg-gray-50">
          ...
          <div className="ml-4 text-sm text-gray-500">Showing {filtered.length} of {devices.length} devices</div>
        </div>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filtered.map(d => (
                <tr key={d.alias} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <div className="font-medium text-gray-900">{d.name || d.alias}</div>
                        <div className="text-xs text-gray-500 font-mono">{d.alias}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-mono">{d.host || '—'}</td>
                  <td className="px-6 py-4 whitespace-nowrap"><StatusBadge status={d.status} /></td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{d.status==='up' && d.latency_ms != null ? `${d.latency_ms} ms` : '—'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{relativeTime(d.checked_at)}</td>
